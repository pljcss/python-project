...
sql = """
select cf.id as config_id,
va.user_id,
to_date(va.create_time) as stat_date,
4 as version,
sum(round(va.amount,2)) as money,
sum(
    case when fp.scope = 28 then round(va.amount,2) * csi.locking_expect / if(csi.locking_expect_unit = 0, 12, 365)
    else round(va.amount,2) * fp.expect / if(fp.expect_unit = 0, 12, 365)
    end) annual_money
from rt.finance_plan fp 
join rt.mkt_invest_ranking_config cf
on cf.stat_start_time <='2019-05-13'
and cf.stat_end_time>='2019-05-13'
join rt.vip_account_mark_ranking_03 va 
on va.fp_id=fp.id
and va.platform = 1
and va.is_cash=0
and to_date(va.create_time)='2019-05-13'
left join rt.invt_custom_trans_info csi 
on csi.trans_id = va.trans_id
where find_in_set(cast(fp.scope as string),cf.stat_product_scope)>=1
group by cf.id,va.user_id,to_date(va.create_time)
"""
